  ...
  return (
    <>
      ...
      {commit.graph.shape === "mergeLeft" ? (
        <span
          className="graph-line diagonal-left"
          style={{ left: `${laneOffset - 16}px`, background: color }}
        />
      ) : null}
      {commit.graph.shape === "mergeRight" ? (
        <span
          className="graph-line diagonal-right"
          style={{ left: `${laneOffset}px`, background: color }}
        />
      ) : null}
